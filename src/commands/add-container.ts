...
const AddContainerImportToApp = async (name: string) => {
  ...
  console.log('replace import Containers: ', results);
}
...
const AddContainerStateImportToActionTypes = async (name: string) => {
  const results = replace.sync({
    files: 'src/util/actionTypes.ts',
    from: '//!!nextstateimport',
    to: "import { " + name + "State } from '../Containers/"+ name + "/" + name + ".codebehind';\n" + 
    "//!!nextstateimport", 
    countMatches: true,
  });
...
export const AddContainerActionTypeToApp = async (name: string) => {
  AddContainerStateImportToActionTypes(name);
...
module.exports = {
  ...
  run: async (toolbox: GluegunToolbox) => {
    ...
    await template.generate({
      ...
      target: `src/Containers/${name}/${name}.codebehind.ts`,
      props: { name }
    })
    ...
    await template.generate({
      template: 'containertests.tsx.ejs',
      target: `__tests__/Containers/${name}.tests.tsx`,
      props: { name }
    })

    // generate the storybook folder for this component from template
    await template.generate({
      template: '/story/container.stories.js.ejs',
